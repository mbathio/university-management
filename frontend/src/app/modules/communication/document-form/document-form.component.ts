...
export class DocumentFormComponent implements OnInit {
  ...
  onFileSelected(event: any): void {
    ...
    if (file) {
      ...
      if (file.size > 10 * 1024 * 1024) {
        this.snackBar.open('Le fichier est trop volumineux. Maximum 10MB.', 'Fermer', {
          duration: 3000
        });
  ...
  onSubmit(): void {
    ...
    if (this.isEditMode && this.documentId) {
      // Mode édition
      this.documentService.updateDocument(this.documentId, formData, this.selectedFile).subscribe({
        next: () => {
