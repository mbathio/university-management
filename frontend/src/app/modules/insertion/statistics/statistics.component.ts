...
export class StatisticsComponent implements OnInit {
  ...
  private loadStatistics(): void {
    ...
    if (year && formationId) {
      ...
      statsObservable = this.insertionService.getInsertionStatisticsByFormation(formationId)
        .pipe(
          catchError(() => this.insertionService.getInsertionStatistics())
        );
    } else if (year) {
      // Filter by year only
      statsObservable = this.insertionService.getInsertionStatisticsByYear(year);
    } else if (formationId) {
